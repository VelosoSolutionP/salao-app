...
export const metadata: Metadata = { title: "Redefinir senha — Veloso Solution" };
...
export default async function RedefinirSenhaPage({ params }: Props) {
  ...
  // Pre-validate token server-side (no-store so it's always fresh)
  let tokenValid = false;
  try {
    const appUrl = process.env.NEXT_PUBLIC_APP_URL ?? "http://localhost:3000";
    const res = await fetch(`${appUrl}/api/auth/redefinir-senha?token=${encodeURIComponent(token)}`, {
      cache: "no-store",
    });
    const json = await res.json();
    tokenValid = json.valid === true;
  } catch {
    // tokenValid stays false — form will show "link inválido"
  }
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="bg-white rounded-3xl shadow-2xl shadow-black/30 overflow-hidden">
          ...
          <div className="p-8 sm:p-10">
            ...
            {/* Heading — only shown when token is valid */}
            {tokenValid && (
              <div className="mb-7">
                <h1 className="text-2xl font-black text-gray-900 tracking-tight">Criar nova senha</h1>
            ...
            <RedefinirSenhaForm token={token} tokenValid={tokenValid} />
          </div>
        </div>
