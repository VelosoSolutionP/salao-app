  ...
  if (
    ...
    try {
      ...
      await resend.emails.send({
        from: process.env.RESEND_FROM ?? "noreply@msbsolution.com.br",
        to: email,
        subject: "Redefinição de senha — MSB Solution",
  ...
  <div style="max-width:480px;margin:40px auto;background:white;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08)">
    ...
    <div style="background:linear-gradient(135deg,#0e0b1a,#1a1040);padding:32px 40px">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div>
          <p style="margin:0;color:white;font-weight:900;font-size:15px">MSB Solution</p>
          <p style="margin:0;color:#7c3aed;font-size:10px;letter-spacing:.1em;text-transform:uppercase">Gestão de Salões</p>
        </div>
      </div>
    </div>
    <!-- Body -->
